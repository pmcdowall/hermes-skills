...
## CRITICAL: op CLI not available on Hermes EC2

The `op` (1Password CLI) binary is NOT installed on the Hermes EC2 (ip-172-31-30-215).
It IS installed on the OpenClaw EC2 (ubuntu@52.63.13.24).

To retrieve MS365 credentials from within a Hermes session or cron job, SSH to the
OpenClaw EC2 first:

```bash
MS365_SECRET=$(ssh -i /home/ubuntu/.ssh/openclaw-ec2.pem -o StrictHostKeyChecking=no ubuntu@52.63.13.24 \
  'export OP_SERVICE_ACCOUNT_TOKEN=$(grep OP_SERVICE_ACCOUNT_TOKEN ~/.bashrc | head -1 | cut -d"'"'"'" -f2) && \
   op item get 7omxmms2z22ac6fd4hdbk5bveq --vault "Paul'"'"'s Vault" --fields label=credential --reveal 2>&1')
```

Note: the OP_SERVICE_ACCOUNT_TOKEN env var is in ~/.bashrc on the OpenClaw EC2 but needs
to be explicitly exported — it does NOT survive a non-interactive SSH session automatically.

## Send email — confirmed working pattern (tested 2026-05-06)

The sendMail endpoint returns HTTP 202 with an EMPTY body on success. An empty curl
response is correct — do not treat it as an error. Always check HTTP status with -w "%{http_code}".

```bash
HTTP_STATUS=$(curl -s -o /dev/null -w "%{http_code}" -X POST \
  "https://graph.microsoft.com/v1.0/users/rocky@onefellswoop.com.au/sendMail" \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    "message": {
      "subject": "Subject here",
      "body": { "contentType": "HTML", "content": "<p>Body here</p>" },
      "toRecipients": [{"emailAddress": {"address": "paul@onefellswoop.com.au"}}]
    }
  }')
# HTTP_STATUS == 202 means sent successfully
```
